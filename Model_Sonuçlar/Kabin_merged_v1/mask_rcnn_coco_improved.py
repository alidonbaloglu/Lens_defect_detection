...
BEST_MODEL_PATH = os.path.join(MODEL_DIR, "MaskRCNN_COCO_yeni_kabin_v1_BEST.pt")
LAST_EPOCH_MODEL_PATH = os.path.join(MODEL_DIR, "MaskRCNN_COCO_yeni_kabin_v1_LAST_EPOCH.pt")
INPUT_MODEL_PATH = os.path.join(MODEL_DIR, "none")
...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Save last epoch model (always update)
    ...
    test_metrics_best = {}
    test_metrics_last = {}
    best_model_test_f1 = 0.0
    last_model_test_f1 = 0.0
    ...
    if test_loader is not None:
        ...
        if os.path.isfile(BEST_MODEL_PATH):
            ...
            best_model_test_f1 = evaluate_pixel_f1(model, test_loader, device)
            test_metrics_best = evaluate_detailed_metrics(
                model, test_loader, device, train_dataset.category_id_to_label
            )
            
            print(f"\n📊 EN İYİ MODEL - Test Seti Sonuçları (Epoch {best_epoch}):")
            print(f"   🎯 Test F1: {best_model_test_f1:.4f}")
            print(f"   📈 Test Precision: {test_metrics_best['overall']['precision']:.4f}")
            print(f"   📈 Test Recall: {test_metrics_best['overall']['recall']:.4f}")
            print(f"   📈 Test IoU: {test_metrics_best['overall']['iou']:.4f}")
            
            # Print per-class metrics for best model
            if test_metrics_best.get('per_class'):
                print(f"\n   📋 Sınıf Başına Metrikler (En İyi Model):")
                for class_name, metrics in sorted(test_metrics_best['per_class'].items()):
                    print(f"      {class_name}:")
        ...
        # Load and test last epoch model
        if os.path.isfile(LAST_EPOCH_MODEL_PATH):
            ...
            last_model_test_f1 = evaluate_pixel_f1(model, test_loader, device)
            test_metrics_last = evaluate_detailed_metrics(
                model, test_loader, device, train_dataset.category_id_to_label
            )
            
            print(f"\n📊 SON EPOCH MODELİ - Test Seti Sonuçları (Epoch {last_epoch_info}):")
            print(f"   🎯 Test F1: {last_model_test_f1:.4f}")
            print(f"   📈 Test Precision: {test_metrics_last['overall']['precision']:.4f}")
            print(f"   📈 Test Recall: {test_metrics_last['overall']['recall']:.4f}")
            print(f"   📈 Test IoU: {test_metrics_last['overall']['iou']:.4f}")
            
            # Print per-class metrics for last model
            if test_metrics_last.get('per_class'):
                print(f"\n   📋 Sınıf Başına Metrikler (Son Epoch):")
                for class_name, metrics in sorted(test_metrics_last['per_class'].items()):
                    print(f"      {class_name}:")
                    print(f"         Precision: {metrics.get('precision', 0):.4f}")
                    print(f"         Recall: {metrics.get('recall', 0):.4f}")
                    print(f"         IoU: {metrics.get('iou', 0):.4f}")
        ...
        print("=" * 80)
        print(f"En İyi Model F1: {best_model_test_f1:.4f}")
        print(f"Son Epoch Model F1: {last_model_test_f1:.4f}")
        print(f"Fark: {abs(best_model_test_f1 - last_model_test_f1):.4f}")
        if best_model_test_f1 > last_model_test_f1:
            print(f"✅ En iyi model daha iyi performans gösteriyor!")
        elif last_model_test_f1 > best_model_test_f1:
            print(f"✅ Son epoch modeli daha iyi performans gösteriyor!")
        else:
            print(f"⚖️  Her iki model aynı performansı gösteriyor!")
    ...
    training_info = {
        ...
        'test_metrics_best_model': {
            'f1_score': best_model_test_f1,
            'overall_metrics': test_metrics_best.get('overall', {}),
            'per_class_metrics': test_metrics_best.get('per_class', {}),
        },
        'test_metrics_last_epoch_model': {
            'f1_score': last_model_test_f1,
            'overall_metrics': test_metrics_last.get('overall', {}),
            'per_class_metrics': test_metrics_last.get('per_class', {}),
        },
