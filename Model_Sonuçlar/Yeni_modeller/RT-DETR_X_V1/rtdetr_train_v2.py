...
DATASET_YAML = r"C:/Users/arge.ortak/Desktop/Lens/Kabin_merged_yolo/data.yaml"
...
BATCH_SIZE = 4 # 8 → 16 (GPU belleği var)
IMAGE_SIZE = 1000  # Kullanıcı tercihi
PATIENCE = 50     # 25 → 50 (Early stopping)
LEARNING_RATE = 0.0001   # 0.001 → 0.0001 (daha düşük, daha stabil)
...
CONF_THRESHOLD = 0.001  # Çok düşük threshold = validation'da tüm predictions hesaplanır
...
AUG_TRANSLATE = 0.05  # 0.1 → 0.05 (daha az)
AUG_SCALE = 0.1   # 0.2 → 0.1 (daha az)
AUG_SHEAR = 0.0
AUG_FLIPUD = 0.0
AUG_FLIPLR = 0.5
AUG_MOSAIC = 0.0  # 0.5 → 0.0 (erken epochlarda kapat)
AUG_MIXUP = 0.0   # 0.1 → 0.0 (erken epochlarda kapat)
...
def evaluate_with_custom_iou(model, data_yaml: str, iou_threshold: float = 0.25):
    ...
    try:
        ...
        # Ultralytics results object - doğru attributes
        map50 = results.box.map50 if hasattr(results.box, 'map50') else (results.results_dict.get('metrics/mAP50(B)', 0.0) if hasattr(results, 'results_dict') else 0.0)
        map50_95 = results.box.map if hasattr(results.box, 'map') else (results.results_dict.get('metrics/mAP50-95(B)', 0.0) if hasattr(results, 'results_dict') else 0.0)
        precision = results.box.mp if hasattr(results.box, 'mp') else (results.results_dict.get('metrics/precision(B)', 0.0) if hasattr(results, 'results_dict') else 0.0)
        recall = results.box.mr if hasattr(results.box, 'mr') else (results.results_dict.get('metrics/recall(B)', 0.0) if hasattr(results, 'results_dict') else 0.0)
