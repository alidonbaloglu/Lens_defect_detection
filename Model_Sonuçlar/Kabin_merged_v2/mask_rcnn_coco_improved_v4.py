...
# ----------------------------
COCO_ROOT = r"C:/Users/arge.ortak/Desktop/Lens/Kabin_merged_v1"
TRAIN_SPLIT = "train"
VAL_SPLIT_NAME = "valid"
TEST_SPLIT = "test"
OUTPUT_DIR = os.path.join("results", "MaskRCNN_v2")
MODEL_DIR = os.path.join("Modeller")
BEST_MODEL_PATH = os.path.join(MODEL_DIR, "MaskRCNN_COCO_kabin_merged_v2_BEST.pt")
LAST_EPOCH_MODEL_PATH = os.path.join(MODEL_DIR, "MaskRCNN_COCO_kabin_merged_v2_LAST_EPOCH.pt")
...
PRINT_FREQ = 100
EARLY_STOP_PATIENCE = 50  # 25 → 20
EARLY_STOP_MIN_DELTA = 0.001  # 0.0005 → 0.001
GRADIENT_CLIP_NORM = 5.0  # 1.0 → 5.0
USE_MIXED_PRECISION = True  # ✅ NEW
TRAINABLE_BACKBONE_LAYERS = 5  # 3 → 4 5
DROPOUT_RATE = 0.2 # ✅ NEW 0,1 0,15 
IOU_THRESHOLD = 0.25  # use this IoU for COCO mAP evaluation 0,20 den 0,50
...
def evaluate_detailed_metrics(model, data_loader, device, category_id_to_label: Optional[Dict[int, int]] = None) -> Dict[str, Dict[str, float]]:
    ...
    if images_seen > 0 and inference_time > 0:
        results['inference_fps'] = float(images_seen / inference_time)
    else:
        results['inference_fps'] = 0.0
    return results


@torch.no_grad()
def evaluate_coco_map(coco_json_path: str, model, data_loader, device, label_to_category_id: Optional[Dict[int, int]] = None) -> Dict:
    """Compute detailed COCOeval statistics for masks.

    Returns a dict containing:
      - 'map_025': AP @ IoU=0.25
      - 'map_50_95': AP averaged over IoU=0.50:0.95 (standard COCO)
      - 'stats_025': dict of COCOeval stats when evaluated at IoU=0.25
      - 'stats_50_95': dict of COCOeval stats for standard evaluation

    If pycocotools is unavailable or no detections, returns zeros.
    """
    ...
    for images, targets in data_loader:
        ...
        for out, tgt in zip(outputs, targets):
            ...
            labels_k = [int(l) for l, k in zip(labels, keep.tolist()) if k]

            for m, sc, lbl in zip(masks, scores_k, labels_k):
                m_bin = (m >= 0.5).astype(np.uint8)
                m_fortran = np.asfortranarray(m_bin)
                rle = coco_mask.encode(m_fortran)
...
def main():
    ...
    epoch_history = []
    # path for per-epoch logging
    per_epoch_dir = os.path.join(OUTPUT_DIR, "epoch_logs")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        ov = val_metrics.get('overall', {})
        for k in ['precision', 'recall', 'f1', 'iou']:
            v = ov.get(k, 0.0)
            if not (0.0 - 1e-6 <= v <= 1.0 + 1e-6):
                print(f"⚠️ {k} değeri aralık dışında: {v}")
        ...
        # mAP sanity checks
        for mv in [val_map_025, val_map_50_95]:
            if not (0.0 - 1e-6 <= mv <= 1.0 + 1e-6):
                print(f"⚠️ mAP değeri aralık dışında: {mv}")
        ...
        # ✅ Write per-epoch JSON log
        per_epoch_path = os.path.join(per_epoch_dir, f"epoch_{epoch+1:03d}.json")
        try:
            with open(per_epoch_path, "w", encoding="utf-8") as f:
                json.dump(epoch_info, f, indent=2, ensure_ascii=False)
        except Exception as e:
            print(f"⚠️ Epoch {epoch+1} JSON kaydı başarısız: {e}")
        ...
        # Save last epoch model (always update every epoch)
        ...
        torch.save(last_epoch_checkpoint, LAST_EPOCH_MODEL_PATH)
    ...
    if test_loader is not None:
        print("\n" + "=" * 80)
        print("TEST SETİ DEĞERLENDİRME - EN İYİ MODEL")
        print("=" * 80)
        ...
        if os.path.isfile(BEST_MODEL_PATH):
            ...
            best_test_f1 = evaluate_pixel_f1(model, test_loader, device)
            best_model_metrics = evaluate_detailed_metrics(
                model, test_loader, device, train_dataset.category_id_to_label
            )
            try:
                best_test_coco = evaluate_coco_map(test_json, model, test_loader, device, train_dataset.label_to_category_id)
                best_model_metrics['map_025'] = best_test_coco.get('map_025', 0.0)
                best_model_metrics['map_50_95'] = best_test_coco.get('map_50_95', 0.0)
                best_model_metrics['coco_stats'] = {
                    'stats_025': best_test_coco.get('stats_025', {}),
                    'stats_50_95': best_test_coco.get('stats_50_95', {})
                }
            except Exception as e:
                print(f"⚠️ Best model test mAP hesaplanamadı: {e}")
                best_model_metrics['map_025'] = 0.0
                best_model_metrics['map_50_95'] = 0.0
                best_model_metrics['coco_stats'] = {}

            print(f"\n📊 EN İYİ MODEL Test Sonuçları (Epoch {best_epoch}):")
            print(f"   🎯 Test F1: {best_test_f1:.4f}")
            print(f"   📈 Test Precision: {best_model_metrics.get('overall',{}).get('precision',0):.4f}")
            print(f"   📈 Test Recall: {best_model_metrics.get('overall',{}).get('recall',0):.4f}")
            print(f"   📈 Test IoU: {best_model_metrics.get('overall',{}).get('iou',0):.4f}")
            print(f"   🧾 Test mAP (IoU={IOU_THRESHOLD}): {best_model_metrics.get('map_025', 0.0):.4f}")
            print(f"   🧾 Test mAP (IoU=0.50:0.95): {best_model_metrics.get('map_50_95', 0.0):.4f}")
            print(f"   ⚡ Test Inference FPS: {best_model_metrics.get('inference_fps', 0.0):.2f}")
            
            # Per-class metrics for best model
            if best_model_metrics.get('per_class'):
                print(f"\n   📋 Sınıf Başına Metrikler (En İyi Model):")
                for class_name, metrics in sorted(best_model_metrics['per_class'].items()):
                    print(f"      {class_name}:")
        ...
        print("\n" + "=" * 80)
        print("TEST SETİ DEĞERLENDİRME - SON EPOCH MODELİ")
        print("=" * 80)
        
        # Evaluate LAST EPOCH model
        if os.path.isfile(LAST_EPOCH_MODEL_PATH):
            checkpoint = torch.load(LAST_EPOCH_MODEL_PATH, map_location=device)
            if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
                model.load_state_dict(checkpoint['model_state_dict'])
            else:
                model.load_state_dict(checkpoint)
            
            last_epoch_info = checkpoint.get('epoch', 'Unknown')
            
            last_test_f1 = evaluate_pixel_f1(model, test_loader, device)
            last_model_metrics = evaluate_detailed_metrics(
                model, test_loader, device, train_dataset.category_id_to_label
            )
            try:
                last_test_coco = evaluate_coco_map(test_json, model, test_loader, device, train_dataset.label_to_category_id)
                last_model_metrics['map_025'] = last_test_coco.get('map_025', 0.0)
                last_model_metrics['map_50_95'] = last_test_coco.get('map_50_95', 0.0)
                last_model_metrics['coco_stats'] = {
                    'stats_025': last_test_coco.get('stats_025', {}),
                    'stats_50_95': last_test_coco.get('stats_50_95', {})
                }
            except Exception as e:
                print(f"⚠️ Last epoch model test mAP hesaplanamadı: {e}")
                last_model_metrics['map_025'] = 0.0
                last_model_metrics['map_50_95'] = 0.0
                last_model_metrics['coco_stats'] = {}

            print(f"\n📊 SON EPOCH MODELİ Test Sonuçları (Epoch {last_epoch_info}):")
            print(f"   🎯 Test F1: {last_test_f1:.4f}")
            print(f"   📈 Test Precision: {last_model_metrics.get('overall',{}).get('precision',0):.4f}")
            print(f"   📈 Test Recall: {last_model_metrics.get('overall',{}).get('recall',0):.4f}")
            print(f"   📈 Test IoU: {last_model_metrics.get('overall',{}).get('iou',0):.4f}")
            print(f"   🧾 Test mAP (IoU={IOU_THRESHOLD}): {last_model_metrics.get('map_025', 0.0):.4f}")
            print(f"   🧾 Test mAP (IoU=0.50:0.95): {last_model_metrics.get('map_50_95', 0.0):.4f}")
            print(f"   ⚡ Test Inference FPS: {last_model_metrics.get('inference_fps', 0.0):.2f}")
            
            # Per-class metrics for last model
            if last_model_metrics.get('per_class'):
                print(f"\n   📋 Sınıf Başına Metrikler (Son Epoch):")
                for class_name, metrics in sorted(last_model_metrics['per_class'].items()):
                    print(f"      {class_name}:")
                    print(f"         Precision: {metrics.get('precision', 0):.4f}")
                    print(f"         Recall: {metrics.get('recall', 0):.4f}")
                    print(f"         F1: {metrics.get('f1', 0):.4f}")
        ...
        # Comparison of best and last models
        print("\n" + "=" * 80)
        print("MODELLER KARŞILAŞTIRMASI")
        print("=" * 80)
        
        if best_model_metrics and last_model_metrics:
            best_f1_test = best_model_metrics.get('overall', {}).get('f1', 0)
            last_f1_test = last_model_metrics.get('overall', {}).get('f1', 0)
            
            print(f"\n📊 F1 Score Karşılaştırması:")
            print(f"   En İyi:    {best_f1_test:.4f} (Epoch {best_epoch})")
            print(f"   Son Epoch: {last_f1_test:.4f}")
            print(f"   Fark:      {abs(best_f1_test - last_f1_test):.4f}")
            
            print(f"\n📊 mAP (IoU={IOU_THRESHOLD}) Karşılaştırması:")
            print(f"   En İyi:    {best_model_metrics.get('map_025', 0):.4f}")
            print(f"   Son Epoch: {last_model_metrics.get('map_025', 0):.4f}")
            
            print(f"\n📊 mAP (IoU=0.50:0.95) Karşılaştırması:")
            print(f"   En İyi:    {best_model_metrics.get('map_50_95', 0):.4f}")
            print(f"   Son Epoch: {last_model_metrics.get('map_50_95', 0):.4f}")
            
            # Determine which model performs best on test
            if best_f1_test > last_f1_test:
                print(f"\n✅ Test setinde en iyi performans: En İyi Model (F1: {best_f1_test:.4f})")
            elif last_f1_test > best_f1_test:
                print(f"\n✅ Test setinde en iyi performans: Son Epoch Modeli (F1: {last_f1_test:.4f})")
            else:
                print(f"\n⚖️  Her iki model aynı performansı gösteriyor (F1: {best_f1_test:.4f})")
    ...
    training_info = {
        ...
        'best_validation': {
            ...
            'map_025': best_val_map,
            'map_50_95': best_val_map_50_95,
            'coco_stats': best_val_coco_stats,
            'inference_fps': best_val_fps,
        },
        'test_metrics_best_model': {
            'f1_score': best_model_metrics.get('overall', {}).get('f1', 0),
            'overall_metrics': best_model_metrics.get('overall', {}),
            'per_class_metrics': best_model_metrics.get('per_class', {}),
            'map_025': best_model_metrics.get('map_025', 0.0),
            'map_50_95': best_model_metrics.get('map_50_95', 0.0),
            'coco_stats': best_model_metrics.get('coco_stats', {}),
            'inference_fps': best_model_metrics.get('inference_fps', 0.0),
        },
        'test_metrics_last_epoch_model': {
            'f1_score': last_model_metrics.get('overall', {}).get('f1', 0),
            'overall_metrics': last_model_metrics.get('overall', {}),
            'per_class_metrics': last_model_metrics.get('per_class', {}),
            'map_025': last_model_metrics.get('map_025', 0.0),
            'map_50_95': last_model_metrics.get('map_50_95', 0.0),
            'coco_stats': last_model_metrics.get('coco_stats', {}),
            'inference_fps': last_model_metrics.get('inference_fps', 0.0),
        },
        'epoch_history': epoch_history,
        'model_paths': {
            'best_model': BEST_MODEL_PATH,
            'last_epoch_model': LAST_EPOCH_MODEL_PATH,
        },
    }
