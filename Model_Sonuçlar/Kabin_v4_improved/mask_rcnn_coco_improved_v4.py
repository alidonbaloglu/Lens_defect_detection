...
TRAINABLE_BACKBONE_LAYERS = 4  # 3 → 4 5
...
def main():
    ...
    if test_loader is not None:
        ...
        # Evaluate BEST model (checkpoint saved as best)
        ...
        # Evaluate LAST model (weights at end of training loop)
        # Reload last state from epoch_history last entry if available
        try:
            last_state_dict = model.state_dict()  # current model is last trained state
            ...
            print(f"⚠️ Last model test değerlendirme hatası: {e}")
            last_model_metrics = {}

        print(f"\n📊 Last Model Test Sonuçları:")
    ...
    training_info = {
        ...
        'test_metrics': best_model_metrics if best_model_metrics else None,
        'test_metrics_last': last_model_metrics if last_model_metrics else None,
