...
def load_model(device: torch.device) -> torchvision.models.detection.MaskRCNN:
    project_root = os.path.dirname(os.path.abspath(__file__))
    weights_path = os.path.join(project_root, "..", "Modeller", "MaskRCNN_COCO_merged_v3.pt") #MaskRCNN_COCO_merged_v3 MaskRCNN_COCO_sinif_v1 
    # fallback to project-local Modeller if relative path different
    if not os.path.exists(weights_path):
        weights_path = os.path.join(project_root, "Modeller", "MaskRCNN_COCO_merged_v3.pt") #MaskRCNN_COCO_merged_v3 MaskRCNN_COCO_sinif_v1

    # Load checkpoint safely for PyTorch 2.6+ and be compatible with older checkpoints
    ckpt = None
    # First try weights_only=True with allowlisted numpy scalar (safer)
    try:
        import numpy as np
        try:
            # Allowlist numpy scalar used by some older pickles
            torch.serialization.add_safe_globals([np._core.multiarray.scalar])  # type: ignore[attr-defined]
        except Exception:
            pass
        ckpt = torch.load(weights_path, map_location=device, weights_only=True)
    except Exception:
        # Fallback: allow full pickle load for trusted local files
        try:
            ckpt = torch.load(weights_path, map_location=device, weights_only=False)  # type: ignore[call-arg]
        except TypeError:
            ckpt = torch.load(weights_path, map_location=device)

    # Determine num_classes if present
    ...
    # Extract a state_dict in a robust way
    state_dict = None
    if isinstance(ckpt, dict):
        if "model_state" in ckpt and isinstance(ckpt["model_state"], dict):
            ...
            # ckpt might already be a raw state_dict
            # Heuristic: dict keys look like parameter names
            sample_keys = list(ckpt.keys())
            if sample_keys and isinstance(sample_keys[0], str):
                state_dict = ckpt

    if state_dict is None:
        raise RuntimeError("Geçersiz checkpoint formatı: state_dict bulunamadı")

    # Strip 'module.' prefixes if saved with DataParallel
    if any(k.startswith("module.") for k in state_dict.keys()):
        state_dict = {k[len("module."):]: v for k, v in state_dict.items()}

    # Load state dict (non-strict to tolerate minor diffs)
...
@torch.no_grad()
def analyze_frame(model, frame_bgr: np.ndarray, score_thr: float = SCORE_THR) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, bool]:
    ...
    return vis, boxes, masks, labels_idx, scores, has_error
    ...
    def on_analyze_saved(self) -> None:
        ...
        for fname in files:
            ...
            try:
                ...
                vis_rgb, boxes, masks, labels_idx, scores, has_error = analyze_frame(self._cnn_model, img, SCORE_THR)
                ...
                if cv2.imwrite(out_path, vis_bgr, [cv2.IMWRITE_PNG_COMPRESSION, 0]):
                    ...
                    # update best per camera by number of boxes (errors)
                    cam_idx = _extract_cam_idx(fname)
                    err_count = int(boxes.shape[0]) if boxes is not None else 0
                    avg_score = float(np.mean(scores)) if (scores is not None and getattr(scores, 'size', 0) > 0) else 0.0
