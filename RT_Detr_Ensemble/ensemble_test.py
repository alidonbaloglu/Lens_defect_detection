...
def main_ensemble(dataset_dir, conf_thresh, iou_thresh, nms_iou_thresh):
    ...
    print(f"\n📂 Görseller taranıyor: {dataset_dir}")
    ...
    print(f"\n📷 Toplam {len(image_files_list)} görsel işlenecek...\n")
    
    all_results = []
    
    for idx, image_path in enumerate(image_files_list, 1):
        image_name = os.path.basename(image_path)
        # image_path örneğin: .../Part1/images/img1.jpg
        # parent_dir: .../Part1/images
        parent_dir = os.path.dirname(image_path)
        
        # Etiket yolunu bulmak için 'images' -> 'labels' değişimi yap
        # Eğer 'images' klasörü yoksa aynı dizinde ara
        if 'images' in os.path.basename(parent_dir).lower():
            # .../Part1/images -> .../Part1/labels
            labels_dir = parent_dir.replace('images', 'labels').replace('IMAGES', 'LABELS')
        else:
            # images klasörü içinde değilse, labels klasörünü aynı seviyede veya sibling olarak ara
            # Basit varsayım: labels dosyası görselle aynı yerde veya bir üstte 'labels' içinde
            labels_dir = parent_dir
            
        label_filename = os.path.splitext(image_name)[0] + '.txt'
        label_path = os.path.join(labels_dir, label_filename)
        
        # Eğer yukarıdaki yöntemle bulunamazsa, belki label aynı yerdedir
        if not os.path.exists(label_path):
             label_path = os.path.join(parent_dir, label_filename)
        ...
        if img is None:
            print(f"  [{idx}/{len(image_files)}] ⚠ Görsel okunamadı: {image_name}")
            continue
        ...
        # Klasör yapısını koruyarak kaydetmek için:
        # dataset_dir: .../PARTLAR
        # image_path: .../PARTLAR/Part1/images/img.jpg
        # rel_path: Part1/images/img.jpg
        try:
            rel_path = os.path.relpath(image_path, dataset_dir)
            # Alt klasörleri oluştur
            output_subdir = os.path.join(images_output_dir, os.path.dirname(rel_path))
            os.makedirs(output_subdir, exist_ok=True)
            output_path = os.path.join(output_subdir, image_name)
        except ValueError:
            # Eğer relpath alınamazsa (farklı sürücü vs) düz kaydet
            output_path = os.path.join(images_output_dir, image_name)
            
        cv2.imwrite(output_path, drawn_img)
        
        # İlerleme göster
        print(f"  [{idx}/{len(image_files_list)}] ✓ {os.path.basename(image_path)} - GT: {len(gt_items)}, Pred: {len(ensemble_preds)}")
    ...
    if all_results:
        ...
        column_order = ["Fotoğraf Adı", "Gerçek Etiket", "Ensemble Tahmin", "Ensemble Confidence", "Skor Detayı", "IoU", "Sonuç", "Model Kaynağı"]
        df = df[column_order]
    ...
    print(f"   └── images/ (Görselleştirmeler)")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source", 
        type=str, 
        default="C:/Users/ali.donbaloglu/Desktop/Lens/Model_test/Yeni/PARTLAR",
