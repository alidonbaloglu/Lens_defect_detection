...
MODEL_YOLO_CLASS_NAMES = ['artik malzeme', 'cizik', 'enjeksiyon noktasi', 'siyah nokta', 'yirtik']
YOLO_CONFIDENCE_THRESHOLD = 0.25
YOLO_IMGSZ = 640

DATASET_DIR = 'C:/Users/ali.donbaloglu/Desktop/Lens/Model_test/dataset/test'
DATASET_YOLO_CLASS_NAMES = ['cizik', 'enjeksiyon_noktasi', 'kirik', 'siyah_nokta', 'siyahlk', 'yabanci']

CLASS_MAPPING = {
    'artik malzeme': ['siyahlk'],
    'cizik': ['cizik'],
    'enjeksiyon noktasi': ['enjeksiyon_noktasi'],
    'siyah nokta': ['siyah_nokta', 'yabanci'],
    'yirtik': ['kirik']
}
...
def main_hibrit():
    ...
    for image_name in image_files:
        ...
        if not gt_items:
            results_list.append({'Fotoğraf': image_name, 'Gerçek Etiket': 'Etiket Yok/Bozuk',
                                 'Aşama 1 Sonucu': 'N/A', 'Aşama 2 (YOLO) Sonucu': 'N/A', 'Nihai Sonuç': 'Belirsiz'})
            continue
        ...
        boxes = outputs[0]['boxes'][high_conf_indices] if high_conf_indices.any() else []
        ...
        for gt_class_id_yolo, gt_box in gt_items:
            ...
            if best_roi_box and max_iou_with_gt > STAGE_1_IOU_THRESHOLD:
                ...
                if x2 > x1 and y2 > y1:
                    ...
                    yolo_outputs = yolo_model.predict(source=cropped_image, conf=YOLO_CONFIDENCE_THRESHOLD, imgsz=YOLO_IMGSZ, verbose=False)
            ...
            if ("Anomali Tespit Edilemedi" in stage1_result or
                "N/A" in yolo_result or
                "Sınıflandırılamadı" in yolo_result or
                "Geçersiz" in yolo_result):
                is_correct = "Belirsiz"

            results_list.append({
                'Fotoğraf': image_name,
                'Gerçek Etiket': ground_truth_label,
                'Aşama 1 Sonucu': stage1_result,
                'Aşama 2 (YOLO) Sonucu': yolo_result,
                'Nihai Sonuç': is_correct
            })

            print(f"{image_name} | Etiket='{ground_truth_label}', IoU={max_iou_with_gt:.2f} -> {is_correct}")
